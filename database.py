...
def ensure_schema():
    """Schema already applied via schema_update.sql — safe no-op."""
    pass
...
def sync_seats_from_bookings():
    """Sync seats.is_booked from actual confirmed bookings — fixes seeded data."""
    conn = get_connection(); cursor = conn.cursor()
...
def get_bookings_for_user(user_id):
    """Get bookings directly by user_id — most reliable method."""
    conn = get_connection(); cursor = conn.cursor()
    cursor.execute("""
        SELECT b.id,
