...
# Force default theme IMMEDIATELY ΓÇö must happen before any Tk/ttk widget is created
...
class LoginWindow(tk.Tk):
    ...
    def _build(self):
        tk.Label(self, text="≡ƒÜî", font=("Segoe UI", 28), bg=BG, fg=YELLOW).pack(pady=(18,2))
        ...
        pw_row.pack(fill="x")
        self.pw_e = tk.Entry(pw_row, font=FONT_BODY, bg=ACCENT, fg=WHITE, insertbackground=WHITE, show="ΓùÅ", width=28)
        self.pw_e.pack(side="left", ipady=5, fill="x", expand=True)
        self._show_pw = False
        tk.Button(pw_row, text="≡ƒæü", bg=ACCENT, fg=WHITE, bd=0, cursor="hand2", command=self._toggle_pw).pack(side="left", padx=(4,0))
        ...
        # ΓöÇΓöÇ Create Account ΓÇö always visible ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
        ...
        tk.Button(ca_frame, text="Γ₧ò  Create Account",
                  bg=YELLOW, fg=BG,
    ...
    def _toggle_pw(self):
        ...
        self.pw_e.config(show="" if self._show_pw else "ΓùÅ")
    ...
    def _sign_in(self):
        ...
        if not username or not password:
            self.err_lbl.config(text="ΓÜá Enter username and password."); return
        user = login_user(username, password, role)
        if not user:
            self.err_lbl.config(text="Γ¥î Invalid credentials or wrong role."); return
        self.destroy()
...
class CreateAccountWindow(tk.Toplevel):
    ...
    def _build_content(self):
        tk.Label(self, text="≡ƒÜî", font=("Segoe UI",26), bg=BG, fg=YELLOW).pack(pady=(16,2))
        ...
        for label, key in [("Password (4-20 chars)","pw"),("Confirm Password","pw2")]:
            tk.Label(card, text=label, bg=PANEL, fg=WHITE, font=FONT_BODY).pack(anchor="w", pady=(4,1))
            e = tk.Entry(card, font=FONT_BODY, bg=ACCENT, fg=WHITE, insertbackground=WHITE, show="ΓùÅ", width=34)
            e.pack(fill="x", ipady=4); self.entries[key] = e
        # Passengers only ΓÇö managers sign in with hardcoded credentials
        self.role_var = tk.StringVar(value="passenger")
        self.err_lbl = tk.Label(card, text="", bg=PANEL, fg=RED, font=FONT_SMALL)
        self.err_lbl.pack(pady=(4,0))
        tk.Button(card, text="Γ£à  Create Account", bg=GREEN, fg=BG,
                  font=("Georgia",11,"bold"), pady=9, cursor="hand2", bd=0,
                  command=self._create).pack(fill="x", pady=(8,0))
        tk.Button(self, text="ΓåÉ Back to Login", bg=BG, fg=YELLOW,
    ...
    def _create(self):
        ...
        if not u: self.err_lbl.config(text="ΓÜá Username required."); return
        if not 4<=len(pw)<=20: self.err_lbl.config(text="ΓÜá Password must be 4-20 chars."); return
        if pw!=p2: self.err_lbl.config(text="ΓÜá Passwords do not match."); return
        ok, result = create_user(u, pw, r, em)
        if not ok: self.err_lbl.config(text=f"Γ¥î {result}"); return
...
class ChangePasswordWindow(tk.Toplevel):
    def __init__(self, parent, user):
        ...
        tk.Label(self, text="≡ƒöæ  Change Password", font=FONT_HEAD, bg=BG, fg=WHITE).pack(pady=(24,2))
        ...
        for label, key in [("Current Password","cur"),("New Password","new"),("Confirm Password","con")]:
            ...
            e = tk.Entry(row, font=FONT_BODY, bg=ACCENT, fg=WHITE, insertbackground=WHITE, show="ΓùÅ", width=28)
            e.pack(side="left", fill="x", expand=True, ipady=5)
            _show=[False]
            def _tog(entry=e, state=_show):
                state[0]=not state[0]; entry.config(show="" if state[0] else "ΓùÅ")
            tk.Button(row, text="≡ƒæü", bg=ACCENT, fg=WHITE, bd=0, cursor="hand2", command=_tog).pack(side="left", padx=2)
            self.entries[key] = e
        self.err_lbl = tk.Label(card, text="", bg=PANEL, fg=RED, font=FONT_SMALL)
        self.err_lbl.pack(pady=(8,0))
        tk.Button(card, text="Γ£à  Update Password", bg=GREEN, fg=BG,
    ...
    def _update(self):
        ...
        if not 4<=len(new)<=20: self.err_lbl.config(text="ΓÜá 4-20 characters required."); return
        if new!=con: self.err_lbl.config(text="ΓÜá Passwords do not match."); return
        ok, msg = change_password(self.user["id"], self.user["role"], cur, new)
        if not ok: self.err_lbl.config(text=f"Γ¥î {msg}"); return
        messagebox.showinfo("Done","Password updated!")
        self.destroy()

def _apply_row_colors(tree):
    """Force row tag colors on Windows ΓÇö must call after tree is packed & visible."""
    # Configure tags directly ΓÇö works with "default" theme
    ...
    tree.tag_configure("highlight",
                       background="#3D1F0D",
                       foreground="#E8A44A")
    # Force Tk to redraw ΓÇö update() is stronger than update_idletasks()
    tree.update()
...
def _ask_password(parent, correct_password, on_success):
    ...
    tk.Label(win,text="≡ƒöÆ  Admin Access Required",bg=PANEL,fg=WHITE,font=("Georgia",13,"bold")).pack(pady=(22,4))
    tk.Label(win,text="Enter password to continue:",bg=PANEL,fg=YELLOW,font=FONT_SMALL).pack()
    pw_var=tk.StringVar()
    pw_e=tk.Entry(win,textvariable=pw_var,show="ΓùÅ",font=FONT_BODY,bg=ACCENT,fg=WHITE,insertbackground=WHITE,width=22)
    ...
    def _check(event=None):
        ...
        else: err.config(text="Γ¥î Incorrect password."); pw_e.delete(0,tk.END)
...
class BusResApp(tk.Tk):
    def __init__(self, user):
        ...
        self.title(f"BusRes ΓÇö {user['username']} ({user['role'].upper()})")
        self.state("zoomed"); self.configure(bg=BG); self.resizable(True,True)
        # Dark theme for all treeviews
        style = ttk.Style(self)
        style.theme_use("default")
        # NO fieldbackground ΓÇö it blocks tag background colors on Windows
        ...
        # ONLY map selected state ΓÇö mapping any other state kills tag colors
        style.map("Treeview",
    ...
    def _build_sidebar(self):
        ...
        # ΓöÇΓöÇ Logo ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
        tk.Label(self.sidebar,text="≡ƒÜî  BusRes",bg=SIDEBAR,fg=WHITE,
        ...
        # ΓöÇΓöÇ User badge ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
        ...
        # ΓöÇΓöÇ Nav items ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
        if self.current_role=="admin":
            nav=[("≡ƒôè  Analytics",       self._show_analytics),
                 ("≡ƒÆ░  Revenue",          self._show_revenue),
                 ("≡ƒÄƒ   Promo Codes",     self._show_promo_codes),
                 ("≡ƒæÑ  User Management",  self._show_user_mgmt),
                 ("≡ƒÜî  All Buses",        self._show_all_buses_admin),
                 ("≡ƒÆ│  Payment Methods",  self._show_payment_methods),
                 ("≡ƒù║   Route Finder",    self._show_routes),
                 ("≡ƒôè  Sort & Stats",     self._show_sort_stats),
                 ("ΓÜÖ   Admin Panel",     self._gate_admin)]
        elif self.current_role=="manager":
            nav=[("≡ƒÜî  Manage Buses",    self._show_manage_buses),
                 ("Γ₧ò  Add Bus",         self._show_add_bus),
                 ("≡ƒÆ¼  Feedback Monitor",self._show_feedback_monitor),
                 ("≡ƒöì  Search",          self._show_search),
                 ("≡ƒù║   Route Finder",   self._show_routes),
                 ("≡ƒôè  Sort & Stats",    self._show_sort_stats)]
        else:
            nav=[("≡ƒÜî  All Buses",       self._show_all_buses_passenger),
                 ("≡ƒÄ½  Book a Seat",      self._show_book_seat),
                 ("≡ƒöì  Search",           self._show_search),
                 ("≡ƒôï  My Bookings",      self._show_my_bookings),
                 ("ΓÅ│  My Waitlist",      self._show_my_waitlist),
                 ("Γ¡É  Feedback",         self._show_feedback),
                 ("≡ƒù║   Route Finder",   self._show_routes)]

        # ΓöÇΓöÇ BOTTOM buttons MUST be packed before nav so they anchor ΓöÇΓöÇ
        tk.Frame(self.sidebar,bg=ACCENT,height=1).pack(side="bottom",fill="x")
        tk.Button(self.sidebar,text="ΓåÆ  Logout",
                  bg="#3D0A0A",fg="#FF6B6B",
                  font=("Segoe UI",10,"bold"),bd=0,pady=11,
                  anchor="w",padx=16,cursor="hand2",
                  command=self._logout).pack(side="bottom",fill="x")
        tk.Frame(self.sidebar,bg=ACCENT,height=1).pack(side="bottom",fill="x")
        tk.Button(self.sidebar,text="≡ƒöæ  Change Password",
                  bg=SIDEBAR,fg=YELLOW,
                  font=("Segoe UI",9,"bold"),bd=0,pady=9,
                  anchor="w",padx=16,cursor="hand2",
                  command=self._change_pw).pack(side="bottom",fill="x")

        # ΓöÇΓöÇ Divider then nav items ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
    ...
    def _show_analytics(self):
        ...
        cards_data=[("≡ƒÜî","Total Buses",stats["total_buses"],GOLD,"%d"),
                    ("≡ƒÄ½","Total Bookings",stats["total_bookings"],GREEN,"%d"),
                    ("≡ƒôê","Avg Fill Rate",float(stats["fill_rate"]),YELLOW,"%.1f%%"),
                    ("≡ƒù║","Active Routes",stats["active_routes"],BLUE,"%d")]
        ...
        _bar(charts_row,"≡ƒôè  Bookings per Bus",get_bookings_per_bus(),"bus_number","bookings",GOLD)
        ...
        _bar(charts_row,"≡ƒôè  Bookings per Route",deduped,"route","bookings",GREEN)
    ...
    def _show_revenue(self):
        ...
        for icon,label,value,color in [("≡ƒÆ░","Total Revenue",f"PKR {stats['total_revenue']:,.2f}",GOLD),
                                        ("≡ƒÄ½","Paid Bookings",str(stats['paid_bookings']),GREEN)]:
    ...
    def _show_promo_codes(self):
        ...
        tree_holder=[None]
        for label,color,cmd in [("≡ƒöä  Refresh",ACCENT,lambda:self._show_promo_codes()),
                                  ("Γ₧ò  New Code",GREEN,self._new_promo_popup),
                                  ("ΓÜí  Toggle",YELLOW,lambda:self._toggle_promo(tree_holder[0])),
                                  ("≡ƒùæ   Delete",RED,lambda:self._delete_promo(tree_holder[0]))]:
        ...
        for p in get_all_promo_codes():
            tree.insert("","end",tags=() if p["active"] else ("inactive",),
                        values=(p["id"],p["code"],f"{p['discount_pct']}%",p["max_uses"],p["used"],"Γ£à Yes" if p["active"] else "Γ¥î No"))
    ...
    def _new_promo_popup(self):
        ...
        tk.Label(win,text="≡ƒÄƒ  New Promo Code",font=FONT_HEAD,bg=BG,fg=WHITE).pack(pady=(20,4))
        ...
        def _create():
            ...
            if not code: err.config(text="ΓÜá Enter a code."); return
            if not disc.isdigit() or not 1<=int(disc)<=100: err.config(text="ΓÜá Discount 1-100."); return
            if not uses.isdigit() or int(uses)<1: err.config(text="ΓÜá Max uses ΓëÑ1."); return
            try: create_promo(code,int(disc),int(uses)); win.destroy(); self._show_promo_codes()
            except Exception as ex: err.config(text=f"Γ¥î {ex}")
        tk.Button(card,text="Γ£à  Create Code",bg=GREEN,fg=BG,font=FONT_BTN,pady=7,cursor="hand2",bd=0,command=_create).pack(fill="x",pady=(10,0))
    ...
    def _show_user_mgmt(self):
        ...
        tb=tk.Frame(self.main,bg=BG); tb.pack(fill="x",padx=24,pady=(4,8))
        tree_ref=[None]
        tk.Button(tb,text="≡ƒöä  Refresh",bg=ACCENT,fg=WHITE,font=FONT_BTN,padx=10,pady=5,
                  cursor="hand2",bd=0,command=self._show_user_mgmt).pack(side="left",padx=4)
        tk.Button(tb,text="≡ƒùæ   Delete Selected",bg=RED,fg=WHITE,font=FONT_BTN,padx=10,pady=5,
    ...
    def _show_all_buses_admin(self):
        ...
        tk.Label(sb, text="≡ƒöì", bg=BG, fg=WHITE, font=FONT_BODY).pack(side="left")
        q_var = tk.StringVar()
        ...
        tk.Button(sb, text="≡ƒöä Refresh", bg=ACCENT, fg=WHITE, font=FONT_BTN,
    ...
    def _show_payment_methods(self):
        ...
        def _refresh():
            ...
            for pm in get_payment_methods():
                ...
                icon="≡ƒÆ│" if "Card" in pm["name"] else "≡ƒÅª" if "Banking" in pm["name"] else "≡ƒô▒" if "Wallet" in pm["name"] else "≡ƒÅº"
                tk.Label(row,text=f"{icon}  {pm['name']}",bg=PANEL,fg=WHITE,font=("Segoe UI",12)).pack(side="left")
                tk.Label(row,text="Enabled Γ£à" if pm["enabled"] else "Disabled Γ¥î",
            ...
            tk.Label(wrap,text="Γôÿ  Changes take effect immediately for new bookings.",bg=BG,fg=YELLOW,font=FONT_SMALL).pack(anchor="w",pady=(12,0))
        _refresh()

    def _show_admin_panel(self):
        self._clear(); self._heading("Admin Panel ΓÇö All Bookings",
                                     "Manage bookings ΓÇó Search ΓÇó Export CSV")
        # ΓöÇΓöÇ Top toolbar: search + export ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
        toolbar=tk.Frame(self.main,bg=BG); toolbar.pack(fill="x",padx=20,pady=(0,4))
        tk.Label(toolbar,text="≡ƒöì",bg=BG,fg=WHITE,font=FONT_SMALL).pack(side="left")
        ...
        def confirm_sel():
            ...
            if messagebox.askyesno("Confirm",f"Confirm booking {bid}?"):
                ...
                if booking and is_seat_taken(booking["seat_id"], bid):
                    messagebox.showwarning("Conflict","Γ¥î This seat is already taken by another confirmed booking!"); return
        ...
        tk.Button(btn_frame,text="Γ¥î  Cancel Selected",bg=RED,fg="white",font=FONT_BTN,cursor="hand2",command=cancel_sel).pack(side="left",padx=8)
        tk.Button(btn_frame,text="Γ£à  Confirm Selected",bg=GREEN,fg="white",font=FONT_BTN,cursor="hand2",command=confirm_sel).pack(side="left",padx=8)

    def _show_waitlist_admin(self):
        self._clear(); self._heading("Waitlist Queue")
        import re as _re
        buses=get_all_buses(); bus_map={f"{b['bus_number']} ΓÇö {b['route']}":b for b in buses}
        ...
        def add_to_waitlist():
            ...
            ok=True
            if not n or len(n)<2: name_err.config(text="ΓÜá Name required."); ok=False
            if not _re.fullmatch(r'\d{5}-\d{7}-\d',c): cnic_err.config(text="ΓÜá CNIC format: 12345-1234567-1"); ok=False
            if not _re.fullmatch(r'0\d{3}-\d{7}',p): phone_err.config(text="ΓÜá Phone format: 0301-1234567"); ok=False
            if not b: bus_err.config(text="ΓÜá Select a bus."); ok=False
            if not ok: return
            if get_passenger_by_cnic(c): cnic_err.config(text="ΓÜá CNIC already registered."); return
        ...
        def serve_next():
            ...
            messagebox.showinfo("Served",f"Γ£à {result['name']} ΓåÆ Seat {result['seat_number']} on {result['bus_number']}"); refresh_list()
        ...
        tk.Button(btn_row,text="Serve Next Γû╢",bg=GREEN,fg=WHITE,font=FONT_BTN,cursor="hand2",command=serve_next).pack(side="left")
        tk.Button(btn_row,text="≡ƒôä Open Log",bg=YELLOW,fg=BG,font=FONT_BTN,cursor="hand2",
    ...
    def _waitlist_popup(self):
        """Full waitlist management in a popup ΓÇö password gated."""
    ...
    def _open_waitlist_popup(self):
        ...
        hdr.pack(fill="x")
        tk.Label(hdr, text="ΓÅ│  Waitlist Management",
                 bg=ACCENT, fg=WHITE, font=("Georgia",14,"bold")).pack(side="left")
        tk.Button(hdr, text="Γ£û  Close", bg=RED, fg=WHITE,
                  font=FONT_BTN, bd=0, cursor="hand2",
                  command=win.destroy).pack(side="right")

        # ΓöÇΓöÇ Queue display ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
        ...
        def refresh_queue():
            ...
            if items:
                ...
                for i, p in enumerate(items):
                    parts.append(
                        f"  {i+1}.  {p['name']}  |  Phone: {p.get('phone','ΓÇö')}  |  "
                        f"Bus: {p.get('bus_number','?')}  |  ID: {p.get('booking_id','?')}")
                queue_lbl.config(
                    text="Queue ΓÇö {} passenger(s) waiting:\n{}".format(
                        len(items), "\n".join(parts)),
                    fg=WHITE)
            else:
                queue_lbl.config(text="Queue is empty.", fg=YELLOW)

        # ΓöÇΓöÇ Add to Waitlist form ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
        ...
        buses   = get_all_buses()
        bus_map = {f"{b['bus_number']} ΓÇö {b['route']}": b for b in buses}
        ...
        def add_to_waitlist():
            ...
            for key, (err_lbl, pattern, msg) in err_labels.items():
                if not vals[key]:
                    err_lbl.config(text="ΓÜá This field is required."); ok = False
                elif not _re.fullmatch(pattern, vals[key]):
                    err_lbl.config(text=f"ΓÜá {msg}"); ok = False
            if not b:
                bus_err.config(text="ΓÜá Please select a bus."); ok = False
            if not ok: return

            if get_passenger_by_cnic(vals["cnic"]):
                err_labels["cnic"][0].config(text="ΓÜá This CNIC is already registered."); return
            ...
            bus_var.set("")
            form_err.config(text=f"Γ£à {vals['name']} added to waitlist!", fg=GREEN)
            refresh_queue()
        ...
        def serve_next():
            ...
            _log("SERVED", passenger["name"], passenger.get("phone",""))
            messagebox.showinfo("Served Γ£à",
                f"{result['name']} ΓåÆ Seat {result['seat_number']} on {result['bus_number']}")
            refresh_queue()
            self._show_admin_panel()
            win.destroy()

        # ΓöÇΓöÇ Action buttons ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
        btn_row = tk.Frame(form, bg=BG)
        btn_row.pack(anchor="w", pady=12)
        tk.Button(btn_row, text="Γ₧ò  Add to Waitlist",
                  bg=ACCENT, fg=WHITE, font=FONT_BTN,
                  cursor="hand2", bd=0, padx=12, pady=7,
                  command=add_to_waitlist).pack(side="left", padx=(0,8))
        tk.Button(btn_row, text="Γû╢  Serve Next",
                  bg=GREEN, fg=BG, font=FONT_BTN,
                  cursor="hand2", bd=0, padx=12, pady=7,
                  command=serve_next).pack(side="left", padx=(0,8))
        tk.Button(btn_row, text="≡ƒôä  Open Log",
    ...
    def _inline_binary(self, search_var, tree):
        ...
        messagebox.showinfo("Binary Search",
            f"Γ£à Seat {val} ΓÇö {bookings[hit]['name']} | Bus: {bookings[hit]['bus_number']} | Status: {bookings[hit]['status']}")
    ...
    def _inline_bst(self, search_var, tree):
        ...
        if r:
            ...
            messagebox.showinfo("BST Search",
                f"Γ£à {r.get('name')} | Seat: {seat_d} | Bus: {r.get('bus_number')} | Status: {r.get('status','N/A')}")
        else:
    ...
    def _show_manage_buses(self):
        ...
        for label,color,cmd in [("≡ƒöä  Refresh",ACCENT,self._show_manage_buses),
                                  ("Γ£Å∩╕Å   Edit",BLUE,lambda:self._edit_bus(tree_ref[0])),
                                  ("≡ƒùæ   Delete",RED,lambda:self._delete_bus_action(tree_ref[0]))]:
    ...
    def _show_edit_bus_form(self,bus_id,vals):
        ...
        def _save():
            ...
            if not all([bn,rt,dt,ts,pr]): err.config(text="ΓÜá All fields required."); return
            if not ts.isdigit() or int(ts)<1: err.config(text="ΓÜá Seats must be ΓëÑ1."); return
            pr_clean = pr.replace("PKR ","").replace(",","").strip()
            if not pr_clean.isdigit() or int(pr_clean)<1: err.config(text="ΓÜá Price must be ΓëÑ1."); return
            pr = pr_clean
            try:
                ...
                err.config(text=f"Γ¥î {ex}")
        btn_row=tk.Frame(form,bg=PANEL); btn_row.pack(anchor="w",pady=12)
        tk.Button(btn_row,text="≡ƒÆ╛  Save",bg=GREEN,fg=BG,font=FONT_BTN,pady=7,cursor="hand2",bd=0,command=_save).pack(side="left",padx=(0,10))
        tk.Button(btn_row,text="Γ£û  Cancel",bg=ACCENT,fg=WHITE,font=FONT_BTN,pady=7,cursor="hand2",bd=0,command=self._show_manage_buses).pack(side="left")
    ...
    def _show_add_bus(self):
        ...
        def _add():
            ...
            if not all([bn,rt,dt,ts,pr]): err.config(text="ΓÜá All fields required."); return
            if not ts.isdigit() or int(ts)<1: err.config(text="ΓÜá Capacity must be greater than 0."); return
            if not pr.isdigit() or int(pr)<1: err.config(text="ΓÜá Price must be ΓëÑ1."); return
        ...
        btn_row=tk.Frame(form,bg=PANEL); btn_row.pack(anchor="w",pady=12)
        tk.Button(btn_row,text="Γ₧ò  Add Bus",bg=GREEN,fg=BG,font=FONT_BTN,pady=7,cursor="hand2",bd=0,command=_add).pack(side="left",padx=(0,10))
        tk.Button(btn_row,text="≡ƒùæ  Clear",bg=ACCENT,fg=WHITE,font=FONT_BTN,pady=7,cursor="hand2",bd=0,command=_clear_form).pack(side="left")

    def _show_feedback_monitor(self):
        self._clear(); self._heading("Feedback Monitoring","View all passenger feedback")
        wrap=self._scrollable()
        tk.Label(wrap,text="Γ¡É  Average Ratings per Bus",font=FONT_HEAD,bg=BG,fg=YELLOW).pack(anchor="w",padx=24,pady=(8,4))
        for r in get_avg_rating_per_bus():
            rating=r["avg_rating"] or 0; stars="Γÿà"*int(rating)+"Γÿå"*(5-int(rating))
            row=tk.Frame(wrap,bg=PANEL,padx=16,pady=6); row.pack(fill="x",padx=24,pady=2)
            tk.Label(row,text=f"{r['bus_number']} ΓÇö {r['route']}",bg=PANEL,fg=WHITE,font=FONT_BODY,width=30,anchor="w").pack(side="left")
        ...
        tk.Label(wrap,text="≡ƒÆ¼  All Feedback",font=FONT_HEAD,bg=BG,fg=YELLOW).pack(anchor="w",padx=24,pady=(16,4))
        ...
        for f in get_all_feedback():
            tree.insert("","end",values=(f["id"],f["passenger"],f["bus_number"],f["route"],"Γÿà"*f["rating"]+"Γÿå"*(5-f["rating"]),f.get("comment",""),str(f["created_at"])[:16]))
    ...
    def _show_search(self):
        ...
        # ΓöÇΓöÇ Hash Search ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
        tk.Label(frame,text="≡ƒöæ  Hash Search ΓÇö Type a Booking ID:",bg=PANEL,fg=WHITE,font=("Georgia",11,"bold")).pack(anchor="w")
        tk.Label(frame,text="Type the ID ΓÇö matching bookings appear below in O(1).",bg=PANEL,fg=YELLOW,font=FONT_SMALL).pack(anchor="w",pady=(0,4))
        ...
        def _update_suggestions(*_):
            ...
            if matches:
                for b in matches[:8]: suggest_lb.insert(tk.END,f"{b['id']}  ΓÇö  {b['name']}")
                suggest_lb.pack(anchor="w",pady=(0,4),in_=hash_sub)
            else: suggest_lb.pack_forget()
        def _on_select(event):
            idx=suggest_lb.curselection()
            if not idx: return
            label=suggest_lb.get(idx[0]); bid=label.split("  ΓÇö  ")[0].strip()
        ...
        def do_hash():
            ...
            if not bid_str.isdigit(): result_lbl.config(text="ΓÜá  Enter a numeric Booking ID."); return
            booking=selected_booking[0] or id_to_booking.get(bid_str)
            if not booking: result_lbl.config(text="Γ¥î  Booking ID not found."); return
            ...
            result_lbl.config(text=f"Γ£à  Hash Found ΓÇö {b.get('name')}  |  Seat: {seat_d}  |  Bus: {b.get('bus_number')}  |  Status: {b.get('status')}")
        ...
        # ΓöÇΓöÇ Binary Search ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
        tk.Label(frame,text="≡ƒöó  Binary Search ΓÇö Enter Seat Number:",bg=PANEL,fg=WHITE,font=("Georgia",11,"bold")).pack(anchor="w")
        ...
        def do_binary():
            ...
            if not val.isdigit(): _bin_write([("ΓÜá  Enter a numeric seat number.","error")]); result_lbl.config(text=""); return
            target=int(val)
            all_b=[b for b in get_all_bookings() if str(b["seat_number"]).isdigit()]
            bookings=sorted(all_b,key=lambda x:int(x["seat_number"]))
            ...
            if hit_idx==-1: _bin_write([(f"Γ¥î  Seat {val} has no booking.","error")]); result_lbl.config(text=""); return
            left=hit_idx
            ...
            lines_tags=[(f"Γ£à  Seat {val} found on {len(hits)} bus(es):\n","header")]
            for b in hits:
                tag="recent" if b["id"]==most_recent_id else "normal"
                star="  Γÿà " if b["id"]==most_recent_id else "  ΓÇó "
                suffix="  ΓåÉ most recent" if b["id"]==most_recent_id else ""
        ...
        tk.Frame(frame,bg=YELLOW,height=1).pack(fill="x",pady=12)
        # ΓöÇΓöÇ BST Search ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
        tk.Label(frame,text="≡ƒî│  BST Search ΓÇö Enter Passenger Name:",bg=PANEL,fg=WHITE,font=("Georgia",11,"bold")).pack(anchor="w")
        ...
        def do_bst():
            ...
            if not name: result_lbl.config(text="ΓÜá Enter a passenger name."); return
            r=self.passenger_bst.search(name)
            if r:
                seat=r.get("seat_number") or "N/A"
                seat_display="No seat yet (waitlisted)" if seat=="N/A" else seat
                result_lbl.config(text=f"Γ£à  BST Found ΓÇö {r.get('name')}  |  Seat: {seat_display}  |  Bus: {r.get('bus_number')}  |  Status: {r.get('status','N/A')}")
            else:
                ...
                if hits:
                    ...
                    for b in hits:
                        ...
                        lines.append(f"  ΓÇó {b['name']}  |  Seat: {seat}  |  Bus: {b['bus_number']}  |  Status: {b['status']}")
                        result_lbl.config(text="Γ£à  Found:\n"+"\n".join(lines))
                else: result_lbl.config(text="Γ¥î  Passenger not found in BST or DB.")
    ...
    def _show_search_filter(self):
        ...
        q_var.trace_add("write",_filter)
        tk.Button(search_frame,text="≡ƒöì Search",bg=YELLOW,fg=BG,font=FONT_BTN,cursor="hand2",bd=0,command=_filter).pack(side="left")
        tree.pack(side="left",fill="x",expand=True); vsb.pack(side="right",fill="y")
    ...
    def _show_routes(self):
        self._clear(); self._heading("Route Finder ΓÇö Dijkstra's Algorithm")
        canvas=tk.Canvas(self.main,bg=BG,highlightthickness=0)
        vsb=ttk.Scrollbar(self.main,orient="vertical",command=canvas.yview)
        ...
        def find():
            ...
            if not f or not t: _lbl(result_frame,"ΓÜá Select both cities.",RED); return
            if f==t: _lbl(result_frame,"ΓÜá From and To must differ.",RED); return
            path,dist=self.route_graph.dijkstra(f,t)
            if dist==float("inf"): _lbl(result_frame,f"Γ¥î No route between {f} and {t}.",RED,bold=True); return
            _lbl(result_frame,"Γ£à  Shortest Route Found",GREEN,13,True)
            _lbl(result_frame,"≡ƒôì  Route Segments:",YELLOW,11,True,(10,2))
            for i in range(len(path)-1):
                seg=self._get_edge_weight(path[i],path[i+1])
                row=tk.Frame(result_frame,bg=PANEL,padx=16,pady=5); row.pack(fill="x",pady=1)
                tk.Label(row,text=f"  Hop {i+1}:  {path[i]}  ΓåÆ  {path[i+1]}",bg=PANEL,fg=WHITE,font=FONT_BODY,width=40,anchor="w").pack(side="left")
            ...
            _lbl(result_frame,"≡ƒöó  Dijkstra distance table:",YELLOW,11,True,(10,2))
            import heapq
            ...
            for node,d in sorted(dist_map.items(),key=lambda x:x[1] if x[1]!=float("inf") else 9999):
                ...
                via=prev.get(node) or "ΓÇö"; d_str=f"{d} km" if d!=float("inf") else "unreachable"
                r=tk.Frame(result_frame,bg=bg_c,padx=16,pady=3); r.pack(fill="x",pady=1)
                tk.Label(r,text=("Γÿà " if is_path else "  ")+node,bg=bg_c,fg=fg_c,font=("Segoe UI",10,"bold" if is_path else ""),width=22,anchor="w").pack(side="left")
            ...
            # BFS section ΓÇö full card so nothing clips
            ...
            tk.Label(bfs_card,
                     text=f"≡ƒöä  BFS Traversal from {f}",
                     bg=PANEL,fg=YELLOW,
            ...
            tk.Label(bfs_card,
                     text=(
                         ...
                         "then moves to the next level ΓÇö like ripples spreading outward from a stone "
    ...
    def _show_sort_stats(self):
        ...
        _section("≡ƒöÇ  Merge Sort  ΓÇö  O(n log n)")
        ...
        _note(f"ΓåÆ How it works: splits the list into halves recursively until each piece has 1 element, then merges pairs back in sorted order. With {n} records it split {merge_merges[0]} times and compared {merge_comparisons[0]:,} pairs during merging.")
        ...
        _section("≡ƒ½º  Bubble Sort  ΓÇö  O(n┬▓)")
        ...
        _note(f"ΓåÆ How it works: repeatedly walks the list comparing adjacent pairs and swapping them if out of order. With {n} records it made {bubble_comparisons[0]:,} comparisons and {bubble_swaps[0]:,} swaps ΓÇö every out-of-order pair costs a swap, which is why it's slow on large data.")
        _row("IDs before (first 5):","  ".join(str(x) for x in data_before),WHITE)
        _row("IDs after  (first 5):","  ".join(str(x) for x in bubble_after),RED)
        _section("ΓÜû∩╕Å  Verdict",GREEN)
        speedup=round(bubble_ms/merge_ms,1) if merge_ms>0 else "Γê₧"
        _row("Speed difference:",f"Merge Sort was {speedup}├ù faster")
        _note("Merge Sort is the industry standard. Bubble Sort is O(n┬▓) ΓÇö doubling data quadruples the work.")
    ...
    def _show_book_seat(self):
        self._clear()
        self._heading("Book a Seat", "Select a bus below ΓÇö FULL buses shown in red join the waitlist")
        import re as _re

        # ΓöÇΓöÇ Two-column layout using PanedWindow so both sides resize ΓöÇΓöÇ
        ...
        # ΓöÇΓöÇ LEFT: Bus table ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
        ...
        # Tag config ΓÇö "default" theme respects these
        tree.tag_configure("full",      background="#5C0000", foreground="#FF6B6B")
        ...
        tk.Label(leg, text=" FULL ΓÇö joins waitlist",
        ...
        # ΓöÇΓöÇ RIGHT: Booking form ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
        ...
        sel_lbl = tk.Label(sel_card, text="ΓåÉ Select a bus from the list",
        ...
        def on_select(event):
            ...
            avail = vals[5]
            status_txt = "≡ƒö┤ FULL ΓÇö will join waitlist" if str(avail)=="0" else f"≡ƒƒó {avail} seats available"
            sel_lbl.config(
                text="Selected: {}\n{}\nDep: {}\n{}  |  {}".format(
        ...
        # Promo code
        tk.Label(right, text="≡ƒÄƒ  Promo Code (optional)",
        ...
        def _check_promo(*_):
            ...
            if p:
                promo_data[0] = p
                promo_lbl.config(text=f"Γ£à {p['discount_pct']}% off!", fg=GREEN)
            else:
                promo_data[0] = None
                promo_lbl.config(text="Γ¥î Invalid", fg=RED)
        ...
        def _show_payment_popup(bus_id_sel, bus_num, bus_route,
            ...
            tk.Label(win, text="≡ƒÆ│  Secure Payment",
            ...
            icons = {"Credit":"≡ƒÆ│","Online":"≡ƒÅª","Mobile":"≡ƒô▒","Bank":"≡ƒÅº"}
            for pm in methods:
                icon = next((v for k,v in icons.items() if k in pm["name"]), "≡ƒÆ░")
            ...
            def _pay():
                ...
                if detail_entry[0]:
                    ...
                    if not detail:
                        err_pay.config(text="ΓÜá Please fill in payment details."); return
                ...
                tk.Label(proc, text="ΓÅ│  Processing payment...",
        ...
        def _show_success(parent_win, name, bus_num, seat,
            ...
            tk.Label(swin, text="Γ£à",  font=("Segoe UI",40), bg=BG, fg=GREEN).pack(pady=(20,4))
            tk.Label(swin, text="Payment Successful!",
                     font=("Georgia",18,"bold"), bg=BG, fg=GREEN).pack()
            tk.Label(swin, text=f"PKR {due:,}  ΓÇó  {method}",
            ...
            tk.Button(btn_row, text="≡ƒÄ½ Ticket PDF",
            ...
            tk.Button(btn_row, text="≡ƒôä Invoice PDF",
    ...
    def _show_my_bookings(self):
        self._clear()
        self._heading("My Bookings", f"Your booking history ΓÇö {self.user['username']}")
        wrap = self._scrollable()
        ...
        if not bookings:
            tk.Label(wrap,
                     text="No bookings yet ΓÇö go to Book a Seat to make your first booking!",
    ...
    def _show_my_waitlist(self):
        self._clear()
        self._heading("My Waitlist", f"Buses you're waiting for ΓÇö {self.user['username']}")
        wrap = self._scrollable()
    ...
    def _show_feedback(self):
        self._clear(); self._heading("Feedback & Reviews",f"Rate your journeys ΓÇö {self.user['username']}")
        wrap=self._scrollable()
        ...
        for i in range(1,6):
            tk.Radiobutton(star_row,text="Γÿà"*i,variable=rating_var,value=i,bg=PANEL,fg=GOLD,
        ...
        def _submit():
            if not pid: sub_err.config(text="ΓÜá No passenger record found for your username."); return
            bk=bus_map.get(bus_var.get())
            if not bk: sub_err.config(text="ΓÜá Select a bus."); return
            comment=comment_e.get("1.0",tk.END).strip()
            submit_feedback(pid,bk["id"],rating_var.get(),comment)
            sub_err.config(text=""); messagebox.showinfo("Thanks","Feedback submitted! Γ¡É"); self._show_feedback()
        tk.Button(sub_frame,text="Γ¡É  Submit Feedback",bg=GREEN,fg=BG,font=FONT_BTN,pady=7,cursor="hand2",bd=0,command=_submit).pack(anchor="w")
        if pid:
            ...
            for f in get_feedback_for_passenger(pid):
                ...
                tk.Label(top_row,text=f"{f['bus_number']} ΓÇö {f['route']}",bg=PANEL,fg=WHITE,font=("Segoe UI",11,"bold")).pack(side="left")
                tk.Label(top_row,text="Γÿà"*f["rating"]+"Γÿå"*(5-f["rating"]),bg=PANEL,fg=GOLD,font=("Segoe UI",12)).pack(side="left",padx=8)
                ...
                tk.Button(card,text="≡ƒùæ Delete",bg=RED,fg=WHITE,font=FONT_SMALL,cursor="hand2",bd=0,command=_del).pack(anchor="w",pady=(6,0))
...
def _generate_ticket_pdf(name, bus_num, seat, pay_data, due, method, disc, promo):
    try:
        ...
        # ΓöÇΓöÇ Header block ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
        header_data = [[
        ...
        # ΓöÇΓöÇ Details table ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
        rows_data = [
        ...
        story.append(Paragraph(
            "BusRes  ΓÇö  Bahria University  |  Thank you for traveling with us!",
...
def _generate_invoice_pdf(name, bus_num, pay_data, due, disc, promo):
    try:
        ...
        # ΓöÇΓöÇ Header: logo left, INVOICE right ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
        hdr = Table([[
        ...
        # ΓöÇΓöÇ Meta info ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
        meta = Table([[
        ...
        # ΓöÇΓöÇ Line items table ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
        item_rows = [[
            Paragraph("Description", sty(10, CREAM_C, bold=True)),
            Paragraph("Amount",      sty(10, CREAM_C, bold=True, align=TA_RIGHT)),
        ],[
            Paragraph(f"Bus Booking  ΓÇö  {bus_num}", sty(10, CREAM_C)),
        ...
        # ΓöÇΓöÇ Payment details row ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
        pay_det = Table([[
        ...
        story.append(Spacer(1, 6*mm))
        story.append(Paragraph(
            "BusRes  ΓÇö  Bahria University  |  Computer-generated invoice.",
            sty(8, ACC_C, align=TA_CENTER)))
